...
The primary dataset was obtained from a 302.08 kW grid-connected photovoltaic (PV) system at Maejo University, Chiang Mai, Thailand ($18.8^\circ$N, $98.9^\circ$E). The dataset spans from November 6, 2021, to October 16, 2025, with a 15-minute temporal resolution, totaling 138,238 time points. Data collection integrates three complementary sources:
...
\begin{itemize}
	...
	\item \textbf{Spatiotemporal Alignment and Upsampling}: To bridge the gap between the synthesized hourly C3S datasets and the high-frequency operational records, a two-stage synchronization process was implemented:
	
	(1) \textbf{Temporal Reconstruction and Alignment}: The synthesized meteorological features, derived from the spatial weighted averaging of ERA5 and ERA5Land tensors, were organized into a continuous hourly time series. By extracting values directly from the chronological time coordinates, this stage ensures that all atmospheric variables are precisely aligned with the local observation window, providing a consistent hourly baseline for all subsequent processing.
	
	(2) \textbf{Temporal Upsampling}: To synchronize the hourly C3S datasets with the 15-minute sampling frequency of the on-site sensors, a \textbf{Cubic Spline Interpolation} was employed. This approach constructs piecewise third-order polynomials, $S_k(t)$, across each hourly interval $[t_k, t_{k+1}]$:
	\begin{equation}
...
This strategy augments baseline features with explicit categorical time variables derived from timestamps:

\begin{equation}
\mathbf{X}_t^{\text{DCE}} = [\mathbf{X}_t^{\text{BF}}, \text{H}(t), \text{M}(t), \text{DayOfWeek}(t), \text{S}(t), \text{IsDaytime}(t)]^\top
\end{equation}

where:
\begin{itemize}
	\small
    \item $\text{H}(t) \in \{0, 1, \ldots, 23\}$: Hour of day
    \item $\text{M}(t) \in \{1, 2, \ldots, 12\}$: Month of year
    \item $\text{DayOfWeek}(t) \in \{0, 1, \ldots, 6\}$: Day of week (0=Monday)
    \item $\text{S}(t) \in \{1, 2, 3, 4\}$: Seasons : 1=Winter (Dec-Feb), 2=Summer (Mar-May), 3=Rainy (Jun-Aug), 4=Post-rainy (Sep-Nov)
    \item $\text{IsDaytime}(t) \in \{0, 1\}$: Binary indicator for daylight hours (06:00-18:00)
\end{itemize}

Categorical variables were one-hot encoded, resulting in 37 total features.

\subsubsection{Strategy 3: Cyclical Encoding (CE)}
Cyclical encoding transforms temporal variables into continuous representations using sine and cosine transformations to preserve periodicity:

\[
\begin{aligned}
\text{HSin}(t) &= \sin\!\big(2\pi\, h(t)/24\big), \\
\text{HCos}(t) &= \cos\!\big(2\pi\, h(t)/24\big), \\[4pt]
\text{DSin}(t) &= \sin\!\big(2\pi\, d(t)/365\big), \\
\text{DCos}(t) &= \cos\!\big(2\pi\, d(t)/365\big).
\end{aligned}
\]
...
\begin{equation}
\mathbf{X}_t^{\text{CE}} = [\mathbf{X}_t^{\text{BF}}, \text{HSin}(t), \text{HCos}(t), \text{DSin}(t), \text{DCos}(t)]^\top
\end{equation}

This representation ensures that 23:59 and 00:01 have similar encodings, addressing discontinuity issues in categorical representations.
...
\begin{equation} 
	\begin{split} 
		&\mathbf{h}T, \mathbf{c}T = \text{LSTM}(\mathbf{X}{t-671:t}) \\ 
		&\mathbf{v}{out} = \mathbf{W}_d \mathbf{h}T + \mathbf{b} \\ &\hat{\mathbf{Y}} = \text{Reshape}(\mathbf{v}{out}) 
	\end{split} 
\end{equation}
Where:
\begin{itemize}
	...
	\item $\mathbf{v}_{out} \in \mathbb{R}^{2880}$ is the flattened output of the Dense layer (calculated as $96 \text{ steps} \times 30 \text{ features}$).
	\item $\hat{\mathbf{Y}} \in \mathbb{R}^{96 \times 30}$ is the final reshaped output matrix representing the full 24-hour forecast horizon at 15-minute resolution.
...
\begin{equation}
	\begin{split}
		&\mathbf{h}_0, \mathbf{c}0 = \text{RNN}(\mathbf{X}{t-671:t}) \\
		&\hat{\mathbf{y}}_1 = \text{Dense}(\mathbf{h}_0)
	\end{split}
\end{equation}

\textbf{2. Recursive Prediction Loop}: For each subsequent step $n \in \{2, \dots, 96\}$, the previous prediction $\hat{\mathbf{y}}_{n-1}$ is used as the current input to update the LSTM cell and generate the next value:
\begin{equation}
	\begin{split}
		&\mathbf{h}n, \mathbf{c}n = \text{LSTMCell}(\hat{\mathbf{y}}{n-1}, [\mathbf{h}{n-1}, \mathbf{c}_{n-1}]) \\
...
To preserve temporal dependencies, the dataset was partitioned chronologically into training, validation, and test sets. Specifically, the initial 60\% of the observations were allocated for model training, while the subsequent 20\% was utilized for validation and hyperparameter tuning. The final 20\% of the time-series data was reserved as an independent test set for final performance evaluation. This split ensures each set contains complete seasonal cycles while maintaining temporal ordering.

\subsubsection{Training Configuration}
All neural models were trained with the following hyperparameters:

\begin{itemize}
    \item \textbf{Optimizer}: Nadam with learning rate 0.001
    \item \textbf{Loss Function}: Mean Squared Error (MSE)
    \item \textbf{Batch Size}: 32
    \item \textbf{Maximum Epochs}: 50
    \item \textbf{Early Stopping}: Patience of 2 epochs on validation loss
    \item \textbf{Regularization}: Dropout rate of 0.3 for CNN/LSTM
\end{itemize}

\subsubsection{Temporal Window Configuration}
Temporal dependencies were captured using a sliding window approach, structured as $(Batch, Time, Features)$ tensors. Each sample comprises a 7-day historical input ($input\_width=672$) across 29 features, mapped to a 24-hour forecast horizon ($label\_width=96$). This configuration results in a total window span of 768 timesteps, effectively bridging week-long temporal patterns with daily generation cycles.
...
\multirow{1}{*}{\textbf{Feat.}} & \multirow{1}{*}{\textbf{MAE}} 
& \textbf{Gain} & \textbf{Best}  \rule{0pt}{10pt} \\ \hline

\multirow{4}{*}{CNN} & 8.24 M & Baseline & 0.0524 & -- &  \rule{0pt}{8pt} \\
 \cline{3-6}

 & 8.24 M & Datetime & 0.0487 & +6.7 &  \rule{0pt}{8pt} \\ 
 \cline{3-6}
 & 8.24 M & Cyclical & \textbf{0.0457} & \textbf{+12.7} & \textbf{Yes} \rule{0pt}{8pt} \\ 
 \cline{3-6}
 & 8.26 M & Lag & 0.0460 & +12.2 &  \rule{0pt}{8pt} \\ 
 \hline
 
\multirow{4}{*}{LSTM} & 0.44 M & Baseline & 0.0423 & -- &  \rule{0pt}{8pt} \\ \cline{3-6}
 & 0.54 M & Datetime & 0.0417 & +2.1 &  \rule{0pt}{8pt} \\ \cline{3-6}
 & 0.60 M & Cyclical & \textbf{0.0409} & \textbf{+4.0} & \textbf{Yes} \rule{0pt}{8pt} \\ \cline{3-6}
 & 1.00 M & Lag & 0.0430 & -0.9 &  \rule{0pt}{8pt} \\ \hline
\multirow{4}{*}{AR-LSTM} & 0.84 M & Baseline & \textbf{0.0401} & -- & \textbf{Yes} \rule{0pt}{8pt} \\ \cline{3-6}
 & 0.89 M & Datetime & 0.0401 & -0.5 &  \rule{0pt}{8pt} \\ \cline{3-6}
 & 0.92 M & Cyclical & 0.0418 & -4.8 &  \rule{0pt}{8pt} \\ \cline{3-6}
 & 0.11 M & Lag & 0.0426 & -6.8 &  \rule{0pt}{8pt} \\ \hline
\end{tabularx}
\end{table}
...
The Convolutional Neural Network (CNN) benefited most substantially from feature engineering, achieving a \textbf{15.1\% MAE reduction} using cyclical encoding. This improvement stems from the synergy between the CNN's convolutional filters and the continuous, smooth input space created by sine/cosine transformations, which effectively represents the periodic nature of solar data.

In contrast, the standard Long Short-Term Memory (LSTM) network showed only modest gains, with a \textbf{4.0\% MAE improvement} from cyclical features. This suggests its internal gating mechanisms are inherently capable of learning temporal dependencies, diminishing the marginal value of explicit feature engineering.

A counter-intuitive result emerged with the Autoregressive LSTM (AR-LSTM), which performed best using only simple baseline features. All engineered feature sets degraded its performance, with lag features increasing MAE by \textbf{6.8\%}. This is attributed to the AR-LSTM's sensitivity to input dimensionality, where extraneous features introduce noise that propagates through its autoregressive loop.

\section{Conclusion}

This study evaluates four distinct temporal feature encoding strategies---raw baseline features, datetime categorical encoding, cyclical sine/cosine transformations, and lag feature generation---for day-ahead PV forecasting in Chiang Mai’s haze-affected tropical monsoon climate. The results demonstrate that the impact of feature engineering is fundamentally architecture-dependent. Among sequence-based neural networks, CNNs achieved the most substantial gains (15.1\% MAE improvement with cyclical encoding), LSTMs showed moderate benefits (4.0\%), while the AR-LSTM performed optimally with simple baseline features---where engineered inputs introduced noise into its autoregressive loop.
