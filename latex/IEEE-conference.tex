...
\begin{document}
	...
	\begin{align*}
		\textbf{Input:}\quad &\mathbf{X}_{t-671:t} \in \mathbb{R}^{672 \times N_{\text{feat}}} \\
		\mathbf{H}                                &= \mathbf{ReLU}\!\big(\mathtt{Conv1D}(\mathbf{X}_{t-671:t},\, k=3,\, f=128)\big) \\
		\mathbf{v}_{\mathtt{out}}                 &= \mathbf{Flatten}(\mathbf{H}) \\
		\hat{\mathbf{y}}                          &= \mathrm{Dense}(\mathbf{v}_{\mathtt{out}}) \\
		\hat{\mathbf{Y}}                          &= \mathbf{Reshape}(\hat{\mathbf{y}}) \in \mathbb{R}^{96 \times 1}
	\end{align*}
	Where:
	\begin{itemize}
		\item $N_{feat}$ varies from the 29-variable baseline to larger sets depending on the chosen temporal encoding strategy.
	\end{itemize}
	...
	\begin{equation*}
		\begin{split}
			\mathbf{h}_T, \mathbf{c}_T &= \mathbf{LSTM}(\mathbf{X}_{t-671:t}; hidden=128) \\
			\mathbf{v}_{out} &= \mathbf{Dense(h_{T})} \\
			\hat{\mathbf{Y}} &= \mathbf{Reshape}(\mathbf{v}_{out}) \in \mathbf{R}^{96 \times 1}
		\end{split}
	\end{equation*}
	Where:
	\begin{itemize}
	...
	\begin{align*}
		\mathbf{h}_0, \mathbf{c}_0 &= \text{RNN}\big(\mathbf{LSTMCell}(\mathbf{X}_{t-671:t}; \text{hidden}\!=\!128)\big) \\
		...
		{\small
			\resizebox{\columnwidth}{!}{%
				\begin{tabular}{|c|c|c|c|c|c|c|}
					...
					\hline
					\multicolumn{1}{|c|}{\shortstack{AR\\LSTM}} & 0.084M & Baseline & 0.0418 & 0.0836 & -2.63 & -- \\
					\cline{2-7}
					& 0.089M & Datetime & 0.0418 & 0.0837 & -3.65 & 0.00 \\
					\cline{2-7}
