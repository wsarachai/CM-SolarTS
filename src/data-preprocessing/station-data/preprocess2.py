...
def fill_with_lagged_data(df, periods_back=35040):
    filled_df = df.copy()
...
def fill_with_future_data(df, periods_forward=35040):
    filled_df = df.copy()

    for col in filled_df.columns:
        # Create a shifted version (one year forward - future data)
        future_series = filled_df[col].shift(-periods_forward)

        # Create condition: where current value is 0 or NaN
        mask = (filled_df[col].isna()) | (filled_df[col] == 0)

        # Fill using the future data where condition is met
        filled_df[col] = filled_df[col].mask(mask, future_series)
...
def display_future_comparison(df, col_name, periods_forward=35040):
    """
    Display original data vs future data for comparison

    Args:
        df: DataFrame with the data
        col_name: Name of the column to compare
        periods_forward: Number of periods to shift forward (default: 1 year for 15-min data)
    """
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8))

    # Plot 1: Original vs Future data
    ax1.plot(df.index, df[col_name], label='Original', alpha=0.7)
    ax1.plot(df.index, df[col_name].shift(-periods_forward), label=f'Future ({periods_forward} periods)', alpha=0.7)
    ax1.set_title(f'Original vs Future Data Comparison - {col_name}')
    ax1.set_xlabel('Time')
    ax1.set_ylabel(col_name)
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ...
    # Plot 2: Recent data zoom (last 960 points)
    recent_data = df[col_name][-960:]
    recent_future = df[col_name].shift(-periods_forward)[-960:]
    ax2.plot(recent_data.index, recent_data, label='Original (Recent)', alpha=0.7)
    ax2.plot(recent_future.index, recent_future, label=f'Future ({periods_forward} periods)', alpha=0.7)
    ax2.set_title(f'Recent Data Comparison - {col_name} (Last 960 points)')
    ax2.set_xlabel('Time')
    ax2.set_ylabel(col_name)
    ax2.legend()
    ...
    # Plot 3: Show where future data is used for filling
    filled_series = df[col_name].copy()
    mask = (df[col_name].isna()) | (df[col_name] == 0)
    filled_series = filled_series.mask(mask, df[col_name].shift(-periods_forward))

    plt.figure(figsize=(12, 6))
    plt.plot(df.index, df[col_name], label='Original', alpha=0.5, color='red')
    plt.plot(df.index, filled_series, label='After Future Filling', alpha=0.7, color='green')
    plt.scatter(df.index[mask], df[col_name].shift(-periods_forward)[mask],
               color='orange', s=10, label='Future Values Used', alpha=0.6)
    plt.title(f'Future Data Filling Effect - {col_name}')
    plt.xlabel('Time')
    plt.ylabel(col_name)
    plt.legend()
    plt.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    csv_path = tf.keras.utils.get_file(origin='https://itsci.mju.ac.th/downloads/watcharin/datasets/pv/device-2.csv.tar.gz')
    ...
    print(f"\nApplying past data filling...")
    filled_df = df.copy()
    for i in range(2):
        print(f"\nFilling iteration {i+1}")
        filled_df = fill_with_lagged_data(filled_df)

    print(f"\nApplying future data filling...")
    future_filled_df = filled_df.copy()
    for i in range(2):
        print(f"\nFilling iteration {i+1} (from end)")
        future_filled_df = fill_with_future_data(future_filled_df)
    ...
    if first_col:
        ...
        display_future_comparison(df, first_col)

    # Save the filled DataFrame to a CSV file
    filename = os.path.join(os.getcwd(), "dataset", "pvdb", "org", "data_2_filled.csv")
    future_filled_df.to_csv(filename, index=True)
    print(f"✅ Saved filled DataFrame to: {filename}")
