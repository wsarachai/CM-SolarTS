...
def extract_all_coordinates(files, lat_point, lon_point, output_csv):
    ...
    for var in combined_df.columns:
        ...
        if non_null > 0:
            ...
            # Format numeric values with 3 decimal places; for non-numeric (e.g., datetime strings), print raw
            try:
                is_numeric = pd.api.types.is_numeric_dtype(combined_df[var])
            except Exception:
                # Fallback if pandas API unavailable for some reason
                is_numeric = isinstance(min_val, (int, float, np.floating, np.integer))

            if is_numeric:
                print(f"  {var}: {non_null}/{total} values, range [{min_val:.3f}, {max_val:.3f}]")
            else:
                print(f"  {var}: {non_null}/{total} values, range [{min_val}, {max_val}]")
