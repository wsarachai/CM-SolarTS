...
class CSVSolarPredictor:
    ...
    def select_features(self, exclude_columns=None):
        ...
        if exclude_columns is None:
            # exclude_columns = [
            #     'Self Consumption',           # เกิดหลัง Current Power (data leakage)
            #     'Self_Consumption_%',         # เกิดหลัง Current Power  
            #     'Grid Feed In',               # ผลลัพธ์ ไม่ใช่ cause
            #     'Internal Power Supply',      # ผลลัพธ์ ไม่ใช่ cause
            #     'External Energy Supply',     # ผลลัพธ์ ไม่ใช่ cause
            #     'datetime',                   # ใช้ derived features แทน
            #     'DayName', 'Year_Quarter',    # redundant กับ features อื่น
            #     'Year',                       # มี cyclical features แทน
            #     'Day',                        # มี DayOfWeek แทน
            #     'Quarter',                    # มี Month และ Season แทน
            #     'Season_Autumn', 'Season_Spring', 'Season_Summer', 'Season_Winter'  # ใช้ Season คอลัมน์เดียวพอ
            # ]
        

            # selected_features = [
            # # ⭐⭐⭐⭐⭐
            # 'Total Irradiation', 'Hour_sin', 'Hour_cos', 'Current_Power_Lag1',
            # # ⭐⭐⭐⭐  
            # 'Current_Power_Lag24', 'Is_Daylight',
            # # ⭐⭐⭐
            # 'Module Temperature', 'Month_sin', 'Month_cos',
            # # ⭐⭐
            # 'Ambient Temperature', 'Current_Power_Rolling_Mean_24h',
            # # ⭐
            # 'is_weekend'
            # ]
            exclude_columns = [
            # 🔴 Data Leakage
            'Self Consumption', 'Self_Consumption_%', 'Grid Feed In', 
            'Internal Power Supply', 'External Energy Supply',
            
            # 🟡 Redundant/ไม่ใช้
            'datetime', 'DayName', 'Year_Quarter', 'Year', 'Day', 'Quarter',
            'Season_Autumn', 'Season_Spring', 'Season_Summer', 'Season_Winter',
            
            # 🔽 Features อื่นๆ ที่ไม่เลือก (ไม่มีดาว)
            'current_power',  # ← ถ้าเป็น target ไม่ต้องใส่เป็น feature
            'Temp_Difference', 'DayOfWeek', 'DayOfYear', 'WeekOfYear',
            'Quarter_sin', 'Quarter_cos', 'DayOfWeek_sin', 'DayOfWeek_cos',
            'Season', 'TimeOfDay', 'Ambient_Temp_Lag1', 'Ambient_Temp_Lag24',
            'Current_Power_Rolling_Mean_3h', 'Ambient_Temp_Rolling_Mean_3h',
            'Ambient_Temp_Rolling_Mean_24h'
            ]
